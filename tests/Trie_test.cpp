...
TEST_F(MapTest, iterator) {
    Trie<int, int> copy_ordered;
    for (auto x : ordered) {
        copy_ordered.insert(x);
    }

    Trie<string, int> copy_string;
...
TEST_F(MapTest, const_iterator) {
    const Trie<int, int> &const_ordered = ordered;
    Trie<int, int> copy_ordered;
    for (auto x : const_ordered) {
        copy_ordered.insert(x);
    }

    const Trie<string, int> &const_string = string_map;
    Trie<string, int> copy_string;
...
TEST_F(MapTest, const_iterator_test) {
    const Trie<int, int> &c_empty = empty;
    const Trie<int, int> &c_ordered = ordered;
    const Trie<int, int> &c_inversed = inversed;
